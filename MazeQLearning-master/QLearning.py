...
class QLearning:
    ...
    def __init__(self, discount_factor, states, rewards, n_states, n_actions):
        """ discount factor = gamma, states and rewards refer to the arrays,
        ...
        self.obj = tkWindow()
        self.RTable = random.randint(10, size=self.obj.maze_width)
    ...
    def update_q_table(self, old_state, action, new_state):
        ...
        # Bellman Equation
        # Q(s, a) = R(s, a) + reward(s') + gamma * max(Q(s', a'))

        current_q = self.QTable[old_state][action]
        value = self.rewards[new_state] + self.gamma * self.QTable[new_state].max()
        self.QTable[old_state][action] = current_q + value
